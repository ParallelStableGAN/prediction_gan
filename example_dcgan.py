...
if opt.dataset in ['imagenet', 'folder', 'lfw']:
    ...
    dataset = dset.ImageFolder(
        root=opt.dataroot, transform=transforms.Compose([
    ...
    dataset = dset.LSUN(
        root=opt.dataroot, classes=['bedroom_train'],
    ...
    dataset = dset.CIFAR10(
        root=opt.dataroot, download=True, transform=transforms.Compose([
            transforms.Resize(opt.imageSize),
            transforms.ToTensor(),
            transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
        ]))
...
if opt.pred:
    print(
        'Prediction of D and G is enabled ('
        'see https://openreview.net/forum?id=Skj8Kag0Z&noteId=rkLymJTSf)'
    )
    lookahead_step = 1.0
else:
    lookahead_step = 0.0
...
for epoch in range(opt.niter):
    for i, data in enumerate(dataloader, 0):
        ...
        print(
            '[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f D(x): %.4f D(G(z)): '
            '%.4f / %.4f'
            % (epoch, opt.niter, i, len(dataloader), errD.data.item(),
